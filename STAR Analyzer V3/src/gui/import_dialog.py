"""
Import dialog for STAR Analyzer V3.
Provides folder selection and progress tracking during import.
Uses threading with queue for thread-safe UI updates.
"""
...
from core.parser import MedPCParser, ParseError, detect_training_stage
...
class ImportDialog(tk.Toplevel):
    ...
    def _run_import(self, folder_path: Path):
        ...
        try:
            ...
            # Fallback to all files if pattern matching finds nothing
            if not files:
                self._queue_update('status', 'No pattern-matched files. Scanning all files...')
                files = self._fallback_file_scan(folder_path)
            ...
            for i, file_path in enumerate(files):
                ...
                try:
                    ...
                    # Only include sessions with identifiable stages
                    stage = detect_training_stage(session.metadata.msn)
                    if stage is not None:
                        self.result.sessions.append(session)
                    else:
                        self.result.errors.append((file_path, 'Could not identify training stage'))
    ...
    def _fallback_file_scan(self, folder_path: Path) -> List[Path]:
        """Fallback file scanning for non-standard naming."""
